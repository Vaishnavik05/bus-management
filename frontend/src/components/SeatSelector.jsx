...
function isSeatBooked(seat) {
  if (typeof seat?.isAvailable === "boolean") {
...
export default function SeatSelector({ route, onClose, inline = false }) {
  ...
  useEffect(() => {
    ...
    async function loadSeats() {
      ...
      try {
        ...
        const busId = getBusId(route);
        if (!busId) throw new Error("Bus is not assigned for this route.");
        const seats = await getRouteSeats(busId);
        if (!mounted) return;
        const normalized = Array.isArray(seats) ? seats : seats?.seats ?? [];
        setSeatsData({ seats: normalized });
      } catch (err) {
        if (!mounted) return;
  ...
  function handleBook() {
    ...
    navigate("/payment", {
      state: { route, selectedSeats, totalAmount: selectedTotal },
    });
    // Do NOT call onBooked here — booking confirmation occurs after payment
  }

  const Card = (
    <div className="modal-card inline-seat-card" role="dialog" aria-modal={!inline}>
      ...
      {loading && !seatsData ? (
        ...
        <>
          <div className="seat-grid" style={{ marginTop: 12 }}>
            {seats.length === 0 ? (
              <div style={{ padding: 20, color: "#607983" }}>No seats available for this bus.</div>
            ) : (
              seats.map((seat) => {
                const key = seatKey(seat);
                ...
                return (
                  <button
                    ...
                    className={
                      booked ? "seat seat-booked" : selectedSeat ? "seat seat-selected" : "seat seat-available"
                    }
                    disabled={booked}
                  >
                    {seat.seatNumber ?? key}
  ...
  if (inline) {
    return <div style={{ display: "flex", justifyContent: "center", marginTop: 18 }}>{Card}</div>;
  }
